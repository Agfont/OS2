...
void removeMoviesAndUsersFromTable(int numberOfMovies, struct Movie* movies, int numberOfUsers, struct User* users) {
    ...
    if(keysToFree == NULL) {
        printf("Memory management failed. Contact professors (RecMatrix)\n");
        exit(1);
    }
...
#define NTHREADS 2
#define NUM_COLS_PER_THREAD 1000
...
typedef struct thread_args {
    ...
    int rowUser;
    struct RecommendationMatrix *recommendationMatrix;
    struct Movie* movies;
    int numberOfMovies;
...
/**
 * Thread: It returns the movieId of the recommended movie. 
 */

void *getRecommendedMovieForUserThread(void *thrd_args) {
    thread_args *arg = (thread_args *) thrd_args;
    thread_return *ret = malloc(sizeof(thread_return));

    ret->recommendedMovieColumn = -1;
    ret->maxScore = -1;
    ...
    int finished = 0;
    int ncolBegin, ncolEnd, ncolTotal;

    ncolTotal = arg->recommendationMatrix->numberOfColumns;

    while (!finished) {
        pthread_mutex_lock(&mutex);
        ncolBegin = arg->ncolCurrent;
        if (ncolBegin >= ncolTotal)
            finished = 1;
        else{
            ncolEnd   = ncolBegin + NUM_COLS_PER_THREAD;
            if (ncolEnd > ncolTotal) 
                ncolEnd = ncolTotal;
            arg->ncolCurrent = ncolEnd;
        }
        ...
        for(int j=ncolBegin; j < ncolEnd; j++) {
            ...
            matrixValue = *(arg->recommendationMatrix->grid + j + arg->rowUser*arg->recommendationMatrix->numberOfColumns);
            if(fabs(matrixValue) <= EPSILON_COMPARISON) {
                currentScore = forecastRating(j, arg->rowUser, arg->recommendationMatrix);
                if(currentScore > ret->maxScore) {
                    ret->maxScore = currentScore;
                    ret->recommendedMovieColumn = j;
                }
            }
        }
    }
...
int getRecommendedMovieForUser(int rowUser, struct RecommendationMatrix *recommendationMatrix, struct Movie* movies, int numberOfMovies) {
    ...
    thread_args thrd_args; /* Only one unique argument for all threads */
    thread_return *thrd_return[NTHREADS]; /* Each thread returns different values */
    ...
    thrd_args.rowUser = rowUser;
    thrd_args.recommendationMatrix = recommendationMatrix;
    thrd_args.movies = movies;
    thrd_args.numberOfMovies = numberOfMovies;
    thrd_args.ncolCurrent = 0;

    /* Create threads */

    for(i = 0; i < NTHREADS; i++) {
        pthread_create(&(ntid[i]), NULL, getRecommendedMovieForUserThread, &thrd_args);
    }

    /* Wait for threads and capture result */

    for(i = 0; i < NTHREADS; i++) 
        pthread_join(ntid[i], (void *) &(thrd_return[i]));

    /* Map-reduce */

    for(i = 0; i < NTHREADS; i++) {
        currentScore = thrd_return[i]->maxScore;
        if (currentScore > maxScore) {
            maxScore = currentScore;
            recommendedMovieColumn = thrd_return[i]->recommendedMovieColumn;
        }
    }
